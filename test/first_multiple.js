...
test('text/html update of :first of multiple selected elements', function (t) {
    ...
    res = hyperglue('<div></div>', { 'div:first': { _text: 'world' } }).innerHTML;
    t.equal(res, '<div>world</div>');

    //fails
    res = hyperglue('<div></div><div></div>', { 'div:first': { _text: 'world' } }).innerHTML;
    t.equal(res, '<div>world</div><div></div>');

    res = hyperglue('<div></div><div></div>', { 'div:first': { 'data-test': 'world' } }).innerHTML;
